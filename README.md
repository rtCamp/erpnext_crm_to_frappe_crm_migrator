...
3. **Child-table reshape.** Restructures children whose schema changes across the source/target pair:
   ...
   - **Customer/Lead bridges for Opportunity.** When an Opportunity references a Customer via `party_name`, a CRM Organization is lazy-created (target-side only — `tabProspect` is never touched) so `CRM Deal.organization` resolves; the original `Customer.name` is also recorded on `CRM Deal.erpnext_customer` if the field is installed (Frappe CRM's ERPNext CRM integration). Likewise, when an Opportunity is owned by a Lead that belongs to a Prospect, `CRM Deal.organization` is populated alongside `CRM Deal.lead`.
